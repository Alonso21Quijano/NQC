    ...
    func draw(parent: Board)
    {
        for kid in parent.children
        {
            if let child = kid as? figures
            {
                ...
                if (parent.showboard.figs[child.x][child.y] as! figures).ID != child.ID || (abs(_: parent.showboard.probability[child.x][child.y]) < 1e-8)  //if figure was removed by otherr, or all desks with it were destroyed
                {
